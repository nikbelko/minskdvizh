    ...
    <div className="container mx-auto px-4 py-8">
      <div className="glass-card p-6 max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-7 gap-1 mb-2">
          {WEEKDAYS.map((d) => (
            <div key={d} className="text-center text-xs font-body text-muted-foreground py-2">{d}</div>
          ))}
        </div>
              ...
              <button
                ...
                className={`relative flex flex-col items-center justify-center py-2.5 rounded-lg text-sm font-body transition-all ${
                  isSelected ? 'bg-primary text-primary-foreground'
                    : isToday ? 'ring-2 ring-accent text-accent font-bold'
                    : 'text-foreground hover:bg-secondary/50'
                }`}
              >
                {format(day, 'd')}
                {hasEvents && (
                  <span className={`absolute bottom-1 w-1.5 h-1.5 rounded-full ${isSelected ? 'bg-primary-foreground' : 'bg-accent'}`} />
                )}
              </button>
            );
          })}
